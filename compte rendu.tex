...
normalfromcauchy <- function(){ #changer le return pour obtenir une réalisation ou le temps mis à l'obtenir#
  ...
  #lancer aléatoire selon la loi de Cauchy standard## 
    x = rcauchy(1)
    
  #lancer aléatoire selon la loi de Bernouilli exprimée précédemment##
    C = rbinom(1,1,(1/2)*(1+x^2)*exp(-(1/2)*x^2)))
...
#s contiendra le nombre de lancers moyens
s = s/10^6
...
#t contiendra tous les lancers effectués, on construit ensuite un histogramme##
hist(t)
...
On obtient bien la densité d'une loi normale standard.
