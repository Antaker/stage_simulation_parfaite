X = matrix(sample(c(-1,1),100,replace = TRUE),10,10)
...
IsingGibbssteps <- function(X,beta,n){ ##effectue n pas de la chaîne de Markov pour le sampler de Gibbs, pour un graphe rectangulaire X##  
...
IsingGibbsconditionnalstop <- function(X,beta,epsilon){ ##effectue un nombre aléatoire de pas de la chaîne de Markov pour le sampler de Gibbs, pour un graphe rectangulaire X##  
  Y=c(mean(X==1))
  bol = TRUE
  i=1
  while (bol){
